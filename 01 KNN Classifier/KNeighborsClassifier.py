...
class KNeighborsClassifier:
    ...
    The predicted class is determined as the most common class 
    for the K neighbors of the sample in the training set.
    ...
    def predict(self, X) -> np.ndarray:
        ...
        if isinstance(X, pd.DataFrame) or isinstance(X, pd.Series):
            X = X.to_numpy()
        predictions = [int(self._predict(x)) for x in X]
        ...
        if self._weights_method == 'uniform':
            ...
            for i in range(self._k):
                ...
                while count > 0:
                    ...
                    count -= 1
            most_common = Counter(labels_weighted).most_common()
